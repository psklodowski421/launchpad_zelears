...
#  end
  ...
  def loading
    ...
    4.times do
      ...
      unless note_1 == [327,328,329,330,331,332,333,334]
        note_1.map!{|add| add+20}
      end
          note_2.each { |note| output.puts(0x90,note,30);sleep(timer)}
      unless note_2 ==[344,343,342,341,340,339,338,337]
        note_2.map!{|add| add+20}
      end
    end
    4.times do
      note_2.each { |note| output.puts(0x80,note,30);sleep(timer)}
      note_2.map!{|add| add-20}
      note_1.each { |note| output.puts(0x80,note,30);sleep(timer)}
      note_1.map!{|add| add-20}
    end
    notes.each do |reset|
